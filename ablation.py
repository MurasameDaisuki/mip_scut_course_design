...
from model.ACC_UXNet_noHANC import ACC_UXNet_noHANC
from model.ACC_UXNet_noUXBlock import ACC_UXNet_noUXBlock
...
print(device)
img_size = 128
model = ACC_UXNet_noUXBlock(h_dim=64,input_size=(img_size,img_size,12)).to(device)# ACC_UXNet_noUXBlock,ResUNet

weights_dict = torch.load('/mnt/disk2/SpineSagT2Wdataset3/weight/model_ablation_01.pth')#
...
if __name__ == '__main__':
    for i in range(2000):
        ...
        trainloss, trainDSC = train(random.sample(range(1, 139),64), model, set_size=16, batch_size=8)
        print("Epochs:", i, "trainloss:", trainloss, "trainDSC:", trainDSC)
        model.eval()
        torch.save(model.state_dict(), '/mnt/disk2/SpineSagT2Wdataset3/weight/model_ablation_01.pth')
        #cross_01: 1, 141
        if i % 3 == 0:
