...
class ACC_UXNet(nn.Module):
    def __init__(self,h_dim=48,input_size=(64,64,64)):
        super(ACC_UXNet, self).__init__()
        H, W, Z = input_size
        scale_factor = 1
        ...
        self.UNetLink0 = nn.Sequential(
            ResBlock3DLink(in_channels=h_dim, inv_factor=1, input_size=[int(H/2), int(W/2), int(Z)]),
        )
        self.UNetLink1 = nn.Sequential(
            ResBlock3DLink(in_channels=h_dim*2, inv_factor=1, input_size=[int(H/4), int(W/4), int(Z)]),
        )
        self.UNetLink2 = nn.Sequential(
            ResBlock3DLink(in_channels=h_dim*4, inv_factor=1, input_size=[int(H/8), int(W/8), int(Z)]),
        )
        self.UNetLink3 = nn.Sequential(
            ResBlock3DLink(in_channels=h_dim*8, inv_factor=1, input_size=[int(H/16), int(W/16), int(Z)]),
        )
        self.UNetLink4 = nn.Sequential(
            ResBlock3DLink(in_channels=h_dim*16, inv_factor=2, input_size=[int(H/32), int(W/32), int(Z)]),
        )
